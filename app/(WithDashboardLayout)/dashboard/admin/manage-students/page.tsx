    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto px-2 sm:px-4 py-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-extrabold tracking-tight text-blue-700 dark:text-blue-300 mb-1">Manage Students</h2>
                    <p className="text-slate-500 dark:text-slate-400 text-sm">Create, edit, and manage all students in the system.</p>
                </div>
                <button
                    onClick={openCreate}
                ...
                <table className="min-w-full text-xs md:text-sm table-fixed">
                    ...
                    <tbody>
                        {isLoading || isFetching ? (
                            ...
                            <tr>
                                <td className="px-4 py-8 text-center text-lg text-slate-400" colSpan={6}>No students found</td>
                            </tr>
                        ) : (
                            students.map((s) => {
                                ...
                                return (
                                    <tr key={id} className="border-t border-slate-100 dark:border-slate-800 hover:bg-blue-50/40 dark:hover:bg-slate-900/40 transition">
                                        <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-100">
                                            <div className="max-w-[200px] sm:max-w-[250px] md:max-w-[300px] truncate" title={s.email}>
                                                {s.email}
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 text-slate-700 dark:text-slate-200 hidden md:table-cell">{s.name}</td>
                                        <td className="px-4 py-3 whitespace-nowrap">
                                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold capitalize ${roleColor(s.role)}`}>
                                                {s.role}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 hidden sm:table-cell whitespace-nowrap">
                                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold capitalize ${statusColor(s.status)}`}>
                                                {s.status}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 hidden lg:table-cell">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${s.isDeleted ? "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300" : "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300"}`}>
                                                {s.isDeleted ? "Yes" : "No"}
                                            </span>
                                        </td>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        value="student"
                                        disabled
                                        className="w-full rounded-lg border border-slate-300 dark:border-slate-700 px-3 py-2 text-base bg-slate-100 text-slate-600 dark:bg-slate-800 dark:border-slate-700"
                                    />
                                </div>
                                <div>
